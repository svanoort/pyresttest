...
DJANGO_PROCESS=$?
sleep 5
python -m cProfile -s tottime pyresttest/resttest.py http://localhost:8000 pyresttest/content-test.yaml > contenttest.txt
...
DJANGO_PROCESS=$?
sleep 5
python -m cProfile -s cumtime pyresttest/resttest.py http://localhost:8000 pyresttest/content-test.yaml > contenttest.txt
kill -9 $DJANGO_PROCESS
